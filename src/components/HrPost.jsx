  ...
  const handleAddPanel = async (e) => {
    ...
    try {
      ...
      if (!panelAssignment.easy || !panelAssignment.intermediate || !panelAssignment.advanced) {
        setError('Please assign all panel members for each level');
        return;
      }
  
      const panelData = {
        post_id: selectedPost.id,
        exam_type: selectedPost.examType,
        panels: [panelAssignment.easy, panelAssignment.intermediate, panelAssignment.advanced]
      };
      ...
      const updatedPosts = posts.map(post => {
        if (post.id === selectedPost.id) {
          return {
            ...post,
            panel: [panelAssignment.easy, panelAssignment.intermediate, panelAssignment.advanced]
          };
        }
        return post;
      });
      ...
      toast.success('Tasks successfully assigned to all panel members!', {
        duration: 4000,
        position: 'top-center',
        icon: '✅',
        style: {
      ...
      // Optionally close the modal after successful assignment
      setIsDetailsModalOpen(false);
      
    } catch (error) {
      console.error('Error updating panel:', error);
      toast.error(error.message || 'Failed to assign panel members');
    }
  };
          ...
          <div className="bg-white/90 rounded-xl p-8 w-full max-w-3xl m-4 shadow-xl">
            ...
            <form onSubmit={isEditMode ? handleEditSubmit : handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Follow-up Questions Count(Minimum mark not Met)</label>
                  <input
                    type="number"
                    name="followupCount"
            ...
            <div className="space-y-6">
              <div className="bg-gray-50 p-6 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-gray-600  font-bold ">Skill required</p>
                    <p className="text-gray-800">{selectedPost.category}</p>
                  </div>
                  <div>
                    <p className="text-gray-600 font-bold">Exam Type</p>
                    <p className="text-gray-800">{selectedPost.examType}</p>
                  </div>
                  <div>
                    <p className="text-gray-600 font-bold">Minimum Experience required</p>
                    <p className="text-gray-800">{selectedPost.minExperience} years</p>
                  </div>
                  <div>
                    <p className="text-gray-600 font-bold"> Test time Allowed</p>
                    <p className="text-gray-800">{selectedPost.totalTime} minutes</p>
                  </div>
                  <div>
                    <p className="text-gray-600 font-bold">Minimum Mark needed(%)</p>
                    <p className="text-gray-800">{selectedPost.coverageNeeded}%</p>
                  </div>
                  <div>
                    <p className="text-gray-600 font-bold">Follow-up Questions(Minimum mark not Met)</p>
              ...
              <div className="bg-gray-50 p-6 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-gray-600 font-bold">Application Deadline</p>
                  ...
                  <div>
                    <p className="text-gray-600 font-bold">Test Start Date</p>
                    <p className="text-gray-800">
              ...
              <div className="bg-gray-50 p-6 rounded-lg">
                <h3 className="font-semibold text-lg mb-4">Task Assignment for Panel Member</h3>
                ...
                {canAssignPanel(selectedPost) ? (
                  <form onSubmit={handleAddPanel} className="space-y-4">
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-700 mb-2">
                          Assignment for Level Intermediate
                        </label>
                        <select
                ...
                {!selectedPost.panel.length && !canAssignPanel(selectedPost) && (
                  <p className="text-sm text-gray-500">
                    No panels have been assigned to this job.
                  </p>
                )}
              </div>
            </div>
